...
desc += '-preset-v2-4gpus'; num_gpus = 4; sched.minibatch_base = 16; sched.minibatch_dict = {4: 512, 8: 256, 16: 128, 32: 64, 64: 32, 128: 16}; sched.G_lrate_dict = {256: 0.0015, 512: 0.002, 1024: 0.003}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 50
...
# train = EasyDict(func='util_scripts.generate_fake_images', run_id=18, num_pngs=1000); num_gpus = 1; desc = 'fake-images-' + str(train.run_id)
# train = EasyDict(func='util_scripts.generate_fake_images', run_id=15, grid_size=[15,8], num_pngs=15, image_shrink=1); num_gpus = 1; desc = 'fake-grids-' + str(train.run_id)
...
# train = EasyDict(func='util_scripts.evaluate_metrics', run_id=15, log='metric-swd-16k.txt', metrics=['swd'], num_images=16384, real_passes=2); num_gpus = 8; desc = train.log.split('.')[0] + '-' + str(train.run_id)
# train = EasyDict(func='util_scripts.evaluate_metrics', run_id=15, log='metric-fid-10k.txt', metrics=['fid'], num_images=10000, real_passes=1); num_gpus = 8; desc = train.log.split('.')[0] + '-' + str(train.run_id)
# train = EasyDict(func='util_scripts.evaluate_metrics', run_id=15, log='metric-fid-50k.txt', metrics=['fid'], num_images=50000, real_passes=1); num_gpus = 8; desc = train.log.split('.')[0] + '-' + str(train.run_id)
# train = EasyDict(func='util_scripts.evaluate_metrics', run_id=15, log='metric-is-50k.txt', metrics=['is'], num_images=50000, real_passes=1); num_gpus = 8; desc = train.log.split('.')[0] + '-' + str(train.run_id)
train = EasyDict(func='util_scripts.evaluate_metrics', run_id=15, log='metric-msssim-20k.txt', metrics=['msssim'], num_images=20000, real_passes=1); num_gpus = 8; desc = train.log.split('.')[0] + '-' + str(train.run_id)
